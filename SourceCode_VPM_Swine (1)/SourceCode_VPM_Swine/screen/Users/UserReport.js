...
import { UserRequestFix, UserRequestReport } from '../../api/Order';
...
class UserReport extends Component {
    ...
    handleDateChange = (event, value) => {
            ...
            if (value !== undefined) {
                ...
                this.setState({ date: fixDate, showDate: false, year: formattedDate.getFullYear()})
            } else {
    ...
    handleRequestFix = () => {
        ...
        let { userid, images, error_code, date, date_end, mc_code, db_name, org_code, typeReport, year } = this.state;
        ...
        // })
        let url ="";
        if(typeReport === "Báo cáo hoạt động")
        {
            url = "Report_GeneratorMachine?org_code=" + org_code + "&start_date=" + date + "&end_date="+ date_end+"&user_create="+userid;
        }
        else if(typeReport === "Báo cáo hư hỏng")
        {
            url = "Report_BrokenMachine?org_code=" + org_code + "&start_date=" + date + "&end_date="+ date_end+"&user_create="+userid;
        }
        else if(typeReport === "Báo cáo hàng năm")
        {
            
            url = "GetReportVPMSwine?userid="+ userid + "&year=" + year + "&db_name=cp_swinesow";//GetReportVPMSwine
        }
        //GetReportVPMSwine
        else
        {
            url = "Report_MaintenanceMachine?org_code=" + org_code + "&start_date=" + date + "&end_date="+ date_end+"&user_create="+userid;
        }
        console.log("url: ", url);
        UserRequestReport(url, res => {
            console.log("org_code: ",org_code);
            console.log("org_code: ",date);
            console.log("org_code: ",date_end);
            console.log("org_code: ",userid);
            console.log("UserRequestReport: ",res);
            if (res === "1") {
                //VpmPushNotification("2", "Có yêu cầu mới cần được duyệt");
                
                //this.setState({ spinner: false });
                Alert.alert("Thông báo", "Thành công! Xin vui lòng kiểm tra mail để xem báo cáo!");
                //this.props.navigation.navigate("Status");
            } else {
                //this.setState({ spinner: false })
                Alert.alert("Thông báo", "Không có báo cáo!")
            }
    ...
    render() {
        ...
        return (
            <Container style={{ backgroundColor: "#f1f1f1" }}>
                <Myheader {...this.props} goBack={false} title="Báo cáo" />
                <Spinner
                ...
                <Content>
                    <View style={{
                        ...
                        <Item picker>
                            <Picker selectedValue={org_code} onValueChange={(value) => this.handleInput("org_code", value)} placeholder="Chọn trại để xem báo cáo" iosIcon={<Icon name="arrow-down" />}>
                                {viewOrg}
                            </Picker>
                        </Item>
                        <Item picker>
                            <Picker selectedValue={typeReport} onValueChange={(value) => this.handleInput("typeReport", value)} placeholder="Loại báo cáo" iosIcon={<Icon name="arrow-down" />}>
                                {viewReport}
                            </Picker>
                        </Item>
                        
                        <Label style={{ marginTop: 30, marginBottom: 10 }}>Ngày bắt đầu</Label>
                        {
                        ...
                        <Label style={{ marginTop: 10, marginBottom: 10 }}>Ngày kết thúc</Label>
                        {
                            Platform.OS === "android" && <Button onPress={this.showDate} success><Icon name="calendar" /><Text>{date_end}</Text></Button>
